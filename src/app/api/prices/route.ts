...
export async function GET(request: Request){
	...
	const wallex = await ky
		.get("https://api.wallex.ir/v1/markets")
		.json<{result: {symbols: Record<string, {
            stats: {
                bidPrice: string
            }
        }> }}>();
    const ramzinex = await ky.get<{open: number}>("https://publicapi.ramzinex.com/exchange/api/v1.0/exchange/chart/statistics-24/2").json()
